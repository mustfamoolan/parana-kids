<x-layout.admin>
    <div class="panel">
        <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h5 class="text-lg font-semibold dark:text-white-light">إضافة مصروف جديد</h5>
        ...
        <form method="POST" action="{{ route('admin.expenses.store') }}" class="space-y-5" x-data="{ expenseType: '{{ old('expense_type', '') }}' }">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="rent" {{ old('expense_type') == 'rent' ? 'selected' : '' }}>إيجار</option>
                        <option value="salary" {{ old('expense_type') == 'salary' ? 'selected' : '' }}>رواتب</option>
                        <option value="other" {{ old('expense_type') == 'other' ? 'selected' : '' }}>صرفيات أخرى</option>
                        <option value="promotion" {{ old('expense_type') == 'promotion' ? 'selected' : '' }}>ترويج</option>
                    </select>
                    @error('expense_type')
                        <div class="mt-1 text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div x-show="expenseType === 'promotion'" class="lg:col-span-2">
                    ...
                    <div id="selected_product" class="mt-2 hidden">
                        <div class="flex items-center justify-between p-2 bg-primary/10 rounded">
                            <span id="selected_product_name" class="text-sm font-medium"></span>
