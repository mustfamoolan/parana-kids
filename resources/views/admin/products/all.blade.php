<x-layout.admin>
        ...
        <div id="activeOrderBanner" class="panel mb-5 !bg-success-light dark:!bg-success/20 border-2 border-success">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <div class="flex gap-2 flex-shrink-0 flex-wrap">
                    <a href="{{ route('admin.carts.view') }}" class="btn btn-info">
                    ...
                    @endif
                    <a href="{{ route('admin.orders.create.start') }}" class="btn btn-danger">
                        <svg class="w-5 h-5 ltr:mr-2 rtl:ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
            ...
            @if(!session('current_cart_id'))
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = '{{ route('admin.orders.create.start') }}';
                    }
                });
                return;
            @endif
        ...
        function loadProductData(productId) {
            ...
            // جلب بيانات المنتج
            fetch(`{{ url('/admin/api/products') }}/${productId}`)
                .then(res => res.json())
                .then(data => {
        ...
        function addSelectedSizesToCart() {
            ...
            // إرسال للسيرفر
            fetch('{{ route('admin.carts.items.store') }}', {
                method: 'POST',
                headers: {
