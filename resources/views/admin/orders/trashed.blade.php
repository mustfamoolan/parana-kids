                ...
                <div class="card-body">
                    ...
                    <form method="GET" class="mb-4">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="search" class="form-control"
                                           ...
                                           placeholder="رقم الطلب، ملاحظات، اسم المندوب، أو اسم المنتج">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                        ...
                        @if(request()->hasAny(['search', 'date_from', 'time_from', 'date_to', 'time_to']))
                            <a href="{{ route('admin.orders.trashed') }}" class="btn btn-secondary">
                            ...
                            <tbody>
                                @forelse($orders as $order)
                                    <tr>
                                        ...
                                        <td>
                                            @if($order->delegate)
                                                <strong>{{ $order->delegate->name }}</strong>
                                                <br><small class="text-muted">{{ $order->delegate->phone }}</small>
                                            @else
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                @if(auth()->user()->role === 'admin')
                                                    <form method="POST" action="{{ route('admin.orders.force-delete', $order) }}"
                                                          class="d-inline"
                                                          onsubmit="return confirm('هل أنت متأكد من الحذف النهائي؟ هذا الإجراء لا يمكن التراجع عنه!')">
                                                        @csrf
                                                        @method('DELETE')
