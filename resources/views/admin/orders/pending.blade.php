    ...
    <div class="panel">
        <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                <a href="{{ route('admin.orders.materials.management', array_merge(request()->all(), ['status' => 'pending'])) }}" class="btn btn-success">
                    <svg class="w-4 h-4 ltr:mr-2 rtl:ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path>
                    </svg>
                    عرض المواد المطلوبة
                </a>
                <a href="{{ route('admin.orders.materials.management-grouped', array_merge(request()->all(), ['status' => 'pending'])) }}" class="btn btn-primary">
                    <svg class="w-4 h-4 ltr:mr-2 rtl:ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            ...
                            @php
                                ...
                                if (request()->filled('supplier_id')) {
                                    $pendingQuery->where('supplier_id', request('supplier_id'));
                                }
...
@endphp
                ...
                <form method="GET" action="{{ route('admin.orders.pending') }}" class="space-y-4">
                    ...
                    <div class="panel bg-gray-50 dark:bg-[#0e1726] p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
                            ...
                            <div class="flex items-center gap-3 text-xs">
                                <button type="button" onclick="selectAllWarehousesPending(true)" class="text-primary dark:text-primary-light hover:underline font-semibold flex items-center gap-1">
                                ...
                                <button type="button" onclick="selectAllWarehousesPending(false)" class="text-danger dark:text-danger-light hover:underline font-semibold flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex flex-wrap gap-3" id="warehouseCheckboxesContainerPending">
                            @php
                            ...
                            @foreach($warehouses as $warehouse)
                                ...
                                <label class="flex items-center gap-2 cursor-pointer bg-white dark:bg-[#1b2e4b] border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-md hover:border-primary transition-colors">
                                    <input
                                        ...
                                        name="warehouse_ids[]"
                                        value="{{ $warehouse->id }}"
                                        class="form-checkbox text-primary warehouse-checkbox-pending"
                                        {{ $isChecked ? 'checked' : '' }}
                                    >
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(request('search') || request('date_from') || request('date_to') || request('time_from') || request('time_to') || request('warehouse_id') || request('confirmed_by') || request('delegate_id') || request('size_reviewed') || request('message_confirmed') || request('hours_filter') || request('supplier_id'))
    ...
    <script>
        function savePendingWarehouseCheckboxes() {
            const checkedCBs = document.querySelectorAll('.warehouse-checkbox-pending:checked');
        ...
        function onPendingWarehouseChange() {
            savePendingWarehouseCheckboxes();
            const form = document.querySelector('form[action*="orders/pending"]') || document.querySelector('form[action*="orders"]');
            if (form) {
                form.submit();
            }
        }

        function selectAllWarehousesPending(select) {
            const checkboxes = document.querySelectorAll('.warehouse-checkbox-pending');
            checkboxes.forEach(cb => {
                cb.checked = select;
            });
            onPendingWarehouseChange();
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.warehouse-checkbox-pending').forEach(cb => {
                    ...
                    try {
                        ...
                        if (Array.isArray(savedIds) && savedIds.length > 0) {
                            ...
                            if (savedParams.toString()) {
                                window.location.href = '{{ route('admin.orders.pending') }}?' + savedParams.toString();
                            }
                        }
                    } catch (e) {}
                }
