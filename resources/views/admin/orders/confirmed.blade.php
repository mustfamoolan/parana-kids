                            ...
                            @php
                                ...
                                if (request()->has('warehouse_ids')) {
                                    ...
                                    if (!empty($selectedWarehouseIds)) {
                                        $confirmedQuery->whereHas('items.product', function ($q) use ($selectedWarehouseIds) {
                                            $q->whereIn('warehouse_id', $selectedWarehouseIds);
                                        });
                                    } else {
                                        $confirmedQuery->whereRaw('1 = 0');
                                    }
...
@endphp
                    ...
                    <div class="panel bg-gray-50 dark:bg-[#0e1726] p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
                            ...
                            <div class="flex items-center gap-3 text-xs">
                                <button type="button" onclick="selectAllWarehousesConfirmed(true)" class="text-primary dark:text-primary-light hover:underline font-semibold flex items-center gap-1">
                                ...
                                <button type="button" onclick="selectAllWarehousesConfirmed(false)" class="text-danger dark:text-danger-light hover:underline font-semibold flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex flex-wrap gap-3" id="warehouseCheckboxesContainerConfirmed">
                            @php
                            ...
                            @foreach($warehouses as $warehouse)
                                ...
                                <label class="flex items-center gap-2 cursor-pointer bg-white dark:bg-[#1b2e4b] border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-md hover:border-primary transition-colors">
                                    <input
                                        ...
                                        name="warehouse_ids[]"
                                        value="{{ $warehouse->id }}"
                                        class="form-checkbox text-primary warehouse-checkbox-confirmed"
                                        {{ $isChecked ? 'checked' : '' }}
                                    >
    ...
    <script>
        function saveConfirmedWarehouseCheckboxes() {
            const checkedCBs = document.querySelectorAll('.warehouse-checkbox-confirmed:checked');
                    ...
                    try {
                        ...
                        if (Array.isArray(savedIds) && savedIds.length > 0) {
                            ...
                            if (savedParams.toString()) {
                                window.location.href = '{{ route('admin.orders.confirmed') }}?' + savedParams.toString();
                            }
                        }
                    } catch (e) {}
                }
