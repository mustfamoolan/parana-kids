        ...
        <div class="mb-5">
            <form method="GET" class="flex flex-wrap gap-4">
                ...
                @if(request()->hasAny(['search', 'date_from', 'date_to', 'time_from', 'time_to']))
                    <div>
                        <a href="{{ route('admin.orders.deleted') }}" class="btn btn-outline-secondary">مسح الفلاتر</a>
                    </div>
                @endif
            </form>
        </div>
                    ...
                    <tbody>
                        @foreach($orders as $order)
                            <tr>
                                ...
                                <td>
                                    @if($order->delegate)
                                        <div>{{ $order->delegate->name }}</div>
                                        <div class="text-xs text-gray-500">{{ $order->delegate->phone }}</div>
                                    @else
                                ...
                                <td>
                                    @if($order->deletedBy)
                                        <div>{{ $order->deletedBy->name }}</div>
                                        <div class="text-xs text-gray-500">{{ $order->deletedBy->role }}</div>
                                    @else
                                        <span class="text-gray-500">غير محدد</span>
                                    @endif
                                </td>
