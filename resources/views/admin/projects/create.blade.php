<x-layout.admin>
    <div>
        ...
        <form method="POST" action="{{ route('admin.projects.store') }}" id="projectForm" class="space-y-5">
            @csrf
            ...
            <div id="investorsSection">
                ...
                <div class="panel">
                    ...
                    <div id="investorsContainer">
                        <!-- سيتم إضافة المستثمرين هنا ديناميكياً -->
                    </div>
    ...
    <script>
        ...
        async function calculateInvestmentValue() {
            const typeSelect = document.getElementById('investment_type');
            const targetsSelect = document.getElementById('investment_targets');
            ...
            if (!type || selectedTargets.length === 0) {
                ...
                calculateRemainingAdminPercentage();
                // تحديث المبالغ عند إعادة تعيين القيمة
            ...
            try {
                ...
                if (remainingInfo && remainingList && selectedTargets.length > 0) {
                    ...
                    selectedTargets.forEach((target) => {
                        const targetData = data.individual_values.find(v => v.id === target.id);
                        if (targetData) {
                            const remaining = targetData.remaining_percentage || 0;
                            const colorClass = remaining > 20 ? 'text-success' : remaining > 10 ? 'text-warning' : 'text-danger';
                            html += `<div class="${colorClass}">${targetData.name}: <strong>${remaining.toFixed(2)}%</strong></div>`;
                        }
                    });
                    remainingList.innerHTML = html || '<div class="text-gray-500">لا توجد معلومات</div>';
        ...
        function validateCostPercentage(investorIndex) {
            ...
            if (costPercentage > 100) {
                if (costPercentageInput) {
                    costPercentageInput.value = '100.00';
                }
            ...
            if (amount > balance && balance > 0) {
                ...
                if (totalValue > 0) {
                    const maxPercentage = (balance / totalValue) * 100;
                    if (costPercentageInput) {
                        costPercentageInput.value = Math.min(maxPercentage, 100).toFixed(2);
                    }
                    calculateInvestmentAmountFromPercentage(investorIndex);
                }
            }
        ...
        // تحديث جميع نسب التكلفة من المبلغ
        function updateAllCostPercentagesFromAmount() {
            const investorRows = document.querySelectorAll('.investor-row');
            investorRows.forEach((row) => {
                const investorIndex = row.getAttribute('data-investor-index');
                calculateCostPercentageFromAmount(parseInt(investorIndex));
            });
        }
        ...
        function calculateRemainingAdminPercentage() {
            ...
            if (adminPercentage < 10) {
                adminElement.className = 'text-2xl font-bold text-red-600 dark:text-red-400';
            } else if (adminPercentage < 20) {
                adminElement.className = 'text-2xl font-bold text-yellow-600 dark:text-yellow-400';
            } else {
            ...
            if (adminCostPercentage < 10) {
                adminCostElement.className = 'text-2xl font-bold text-red-600 dark:text-red-400';
            } else if (adminCostPercentage < 20) {
                adminCostElement.className = 'text-2xl font-bold text-yellow-600 dark:text-yellow-400';
            } else {
                adminCostElement.className = 'text-2xl font-bold text-green-600 dark:text-green-400';
            }
        }
