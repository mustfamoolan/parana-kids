            ...
            <div class="panel">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="project_name" name="project_name" value="{{ old('project_name', $project->name) }}" class="form-input @error('project_name') border-red-500 @enderror" required>
                        @error('project_name')
                ...
                <div class="panel mb-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="number" id="total_value_display" name="investment[total_value]" step="0.01" min="0" class="form-input" value="{{ old('investment.total_value', $investment->total_value ?? 0) }}" readonly>
                            <small class="text-gray-500">سيتم تحديثها تلقائياً عند إضافة منتجات للمخزن (للاطلاع فقط)</small>
                        </div>
                    </div>
                </div>
                    ...
                    <div id="investorsContainer">
                        @if($investment && $investment->investors)
                            @foreach($investment->investors as $index => $investmentInvestor)
                                <div class="investor-row panel mb-4" data-investor-index="{{ $index }}" data-investor-id="{{ $investmentInvestor->investor_id }}">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <select name="investment[investors][{{ $index }}][investor_id]" class="form-select investor-select" required onchange="loadInvestorBalance({{ $index }}, this.value)">
                                                ...
                                                @foreach($investors as $investor)
                                                    <option value="{{ $investor->id }}" {{ $investmentInvestor->investor_id == $investor->id ? 'selected' : '' }}>
                                                        {{ $investor->name }}{{ $investor->is_admin ? ' (المدير)' : '' }} ({{ $investor->phone }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div>
                                            ...
                                            <input type="text" id="available_balance_{{ $index }}" class="form-input" readonly style="background-color: #f3f4f6;" value="{{ number_format($investmentInvestor->investor->treasury->current_balance ?? 0, 2) }} IQD">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="investment[investors][{{ $index }}][cost_percentage]" step="0.01" min="0" max="100" class="form-input cost-percentage-input" required value="{{ old("investment.investors.$index.cost_percentage", $investmentInvestor->cost_percentage ?? 0) }}" oninput="calculateRemainingAdminPercentage()" onchange="calculateRemainingAdminPercentage()">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="investment[investors][{{ $index }}][profit_percentage]" step="0.01" min="0" max="100" class="form-input profit-percentage-input" required value="{{ old("investment.investors.$index.profit_percentage", $investmentInvestor->profit_percentage ?? 0) }}" oninput="calculateRemainingAdminPercentage()">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
