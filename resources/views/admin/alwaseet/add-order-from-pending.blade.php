<x-layout.admin>
    <div class="panel">
        <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h5 class="text-lg font-semibold dark:text-white-light">إضافة طلب الوسيط</h5>
            ...
            <!-- فلتر وبحث -->
            <div class="mb-5">
                <form method="GET" action="{{ route('admin.alwaseet.add-order-from-pending') }}" class="space-y-4">
                    <!-- الصف الأول: البحث -->
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(request('search') || request('date_from') || request('date_to') || request('time_from') || request('time_to') || request('warehouse_id') || request('confirmed_by') || request('delegate_id') || request('size_reviewed') || request('message_confirmed'))
                                <a href="{{ route('admin.alwaseet.add-order-from-pending') }}" class="btn btn-outline-secondary" id="clearFiltersBtn">
                                    <svg class="w-4 h-4 ltr:mr-2 rtl:ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($orders as $index => $order)
                        ...
                        <div id="order-{{ $order->id }}" class="panel border-2 border-yellow-500 dark:border-yellow-600">
                            ...
                            @php
                                $missingFields = [];
                                if (empty($order->alwaseet_city_id)) {
                                    $missingFields[] = 'محافظة';
                                }
                                if (empty($order->alwaseet_region_id)) {
                                    $missingFields[] = 'المنطقة';
                                }
                            @endphp
                            @if(count($missingFields) > 0)
                                <div class="mb-4">
                                    <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-3 rounded-lg">
                                        <span class="text-xs font-semibold text-red-700 dark:text-red-400 block mb-2">
                            ...
                            <div class="mb-4">
                                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-3 rounded-lg">
                                    ...
                                    <!-- زر إرسال الطلب للواسط -->
                                    <button
                                        type="button"
                                        onclick="sendOrderToAlWaseet({{ $order->id }})"
                            ...
                            <div class="flex gap-2 flex-wrap">
                                @php
                                    $backRoute = 'admin.alwaseet.add-order-from-pending';
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const filters = [
                { id: 'warehouseFilterPending', key: 'selectedWarehouse_alwaseet_pending', param: 'warehouse_id' },
                { id: 'searchFilterPending', key: 'selectedSearch_alwaseet_pending', param: 'search' },
                { id: 'confirmedByFilterPending', key: 'selectedConfirmedBy_alwaseet_pending', param: 'confirmed_by' },
                { id: 'delegateIdFilterPending', key: 'selectedDelegateId_alwaseet_pending', param: 'delegate_id' },
                { id: 'sizeReviewedFilterPending', key: 'selectedSizeReviewed_alwaseet_pending', param: 'size_reviewed' },
                { id: 'messageConfirmedFilterPending', key: 'selectedMessageConfirmed_alwaseet_pending', param: 'message_confirmed' },
                { id: 'dateFromFilterPending', key: 'selectedDateFrom_alwaseet_pending', param: 'date_from' },
                { id: 'dateToFilterPending', key: 'selectedDateTo_alwaseet_pending', param: 'date_to' },
                { id: 'timeFromFilterPending', key: 'selectedTimeFrom_alwaseet_pending', param: 'time_from' },
                { id: 'timeToFilterPending', key: 'selectedTimeTo_alwaseet_pending', param: 'time_to' }
            ];
            ...
            if (!hasUrlParams && hasSavedFilters && savedParams.toString()) {
                const form = document.querySelector('form[action*="add-order-from-pending"]');
                if (form) {
