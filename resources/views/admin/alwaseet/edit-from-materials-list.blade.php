<x-layout.admin>
    <div class="panel">
        <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h5 class="text-lg font-semibold dark:text-white-light">تعديل طلب #{{ $order->order_number }}</h5>
            <a href="{{ route('admin.alwaseet.materials-list', $backParams) }}" class="btn btn-outline-primary">
        ...
        <form method="POST" action="{{ route('admin.alwaseet.materials-list.orders.update', $order) }}">
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="panel">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="client_name" id="client_name" value="{{ old('client_name', $shipment->client_name ?? $order->customer_name) }}"
                               class="form-input" required>
                    </div>

                    <div class="mb-4">
                        <label for="client_mobile" class="mb-2 block text-sm font-medium">رقم الهاتف <span class="text-danger">*</span></label>
                        <input type="text" name="client_mobile" id="client_mobile" value="{{ old('client_mobile', $shipment->client_mobile ?? $order->customer_phone) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="client_mobile2" id="client_mobile2" value="{{ old('client_mobile2', $shipment->client_mobile2 ?? $order->customer_phone2) }}"
                ...
                <div class="panel">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="city_id" id="city_id" class="form-select" required>
                            ...
                            @foreach($cities as $city)
                                <option value="{{ $city['id'] }}" {{ old('city_id', $shipment->city_id ?? $order->alwaseet_city_id) == $city['id'] ? 'selected' : '' }}>
                                    {{ $city['city_name'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="region_id" id="region_id" class="form-select" required>
                            ...
                            @foreach($regions as $region)
                                <option value="{{ $region['id'] }}" {{ old('region_id', $shipment->region_id ?? $order->alwaseet_region_id) == $region['id'] ? 'selected' : '' }}>
                                    {{ $region['region_name'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="location" id="location" rows="3" class="form-textarea" required>{{ old('location', $shipment->location ?? $order->customer_address) }}</textarea>
                    </div>
                </div>
                ...
                <div class="panel">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $shipment->price ?? $order->total_amount) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="items_number" id="items_number" value="{{ old('items_number', $shipment->items_number ?? $order->items->sum('quantity') ?? '1') }}"
            ...
            <div class="mt-5 flex gap-2">
                ...
                <a href="{{ route('admin.alwaseet.materials-list', $backParams) }}" class="btn btn-outline-secondary">إلغاء</a>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        document.getElementById('city_id').addEventListener('change', function() {
            ...
            .then(data => {
                if (data.regions) {
                    const currentRegionId = '{{ $shipment->region_id ?? $order->alwaseet_region_id }}';
