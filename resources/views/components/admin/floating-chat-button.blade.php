@if(auth()->check() && auth()->user()->isAdminOrSupplier() && request()->is('admin/*'))
    <div class="fixed top-1/2 -translate-y-1/2 ltr:right-4 rtl:left-4 z-50">
        <a href="{{ route('chat.index') }}"
           class="rounded-full bg-primary text-white shadow-lg hover:bg-primary/90 dark:hover:bg-primary/80 transition-all p-2 flex items-center justify-center">
