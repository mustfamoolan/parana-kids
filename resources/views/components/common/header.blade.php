                ...
                <div class="dropdown flex-shrink-0" x-data="dropdown" @click.outside="open = false">
                    <a href="javascript:;" class="relative group" @click="toggle()">
                        <span><img class="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100"
                                src="{{ auth()->check() ? auth()->user()->getProfileImageUrl() : '/assets/images/user-profile.jpeg' }}" alt="{{ auth()->check() ? auth()->user()->name : 'User' }}" /></span>
                    </a>
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                            ...
                            <div class="flex items-center px-4 py-4">
                                <div class="flex-none">
                                    <img class="rounded-md w-10 h-10 object-cover"
                                        ...
                                        alt="{{ auth()->check() ? auth()->user()->name : 'User' }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4 truncate">
                                    <h4 class="text-base">{{ auth()->check() ? auth()->user()->name : 'Guest' }}<span
                                    ...
                                    <a class="text-black/60  hover:text-primary dark:text-dark-light/60 dark:hover:text-white"
                                        href="javascript:;">{{ auth()->check() ? (auth()->user()->email ?? auth()->user()->phone) : 'غير مسجل' }}</a>
                                </div>
                            </div>
                        </li>
                        <li>
                        ...
                        <li class="border-t border-white-light dark:border-white-light/10">
                            ...
                            @elseif(auth()->check() && auth()->user()->isDelegate())
                                <form method="POST" action="{{ route('delegate.logout') }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-danger !py-3 w-full text-left" @click="toggle">
                                        <svg class="w-4.5 h-4.5 ltr:mr-2 rtl:ml-2 shrink-0 rotate-90" width="18" height="18"
                                            viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
