...
class MessageApiController extends Controller
{
    ...
    /**
     * الحصول على أو إنشاء محادثة مع مستخدم
     */
    ...
    public function getMessages(Request $request, $conversationId)
    {
        ...
        try {
            ...
            Log::error('MessageApiController: Error deleting conversation alerts: ' . $e->getMessage());
        }
    ...
    public function sendMessage(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                Log::error('MessageApiController: Error sending SweetAlert: ' . $e->getMessage());
            }
            ...
            Log::error('MessageApiController: Error sending message: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function sendMessageToUser(Request $request)
    {
        ...
        try {
            ...
            Log::error('MessageApiController: Error sending message to user: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'حدث خطأ في إرسال الرسالة: ' . $e->getMessage()
            ], 500);
     ...
     */
    public function addParticipantsToGroup(Request $request, $conversationId)
    {
        $user = Auth::user();
        ...
        if (!$user->isAdmin()) {
            return response()->json([
                'success' => false,
                'message' => 'غير مصرح - المدير فقط يمكنه إضافة مستخدمين'
            ], 403);
        }
     ...
     */
    public function removeParticipantFromGroup(Request $request, $conversationId, $userId)
    {
        $user = Auth::user();
        ...
        if (!$user->isAdmin()) {
            return response()->json([
                'success' => false,
                'message' => 'غير مصرح - المدير فقط يمكنه إزالة مستخدمين'
            ], 403);
        }
     ...
     */
    public function getGroupParticipants($conversationId)
    {
        $user = Auth::user();
