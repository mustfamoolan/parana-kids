...
class MobileAdminOrderController extends Controller
{
    ...
    /**
     * جلب قائمة الطلبات المؤهلة للإرجاع الجزئي (المقيدة)
     *
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function getPartialReturns(Request $request)
    {
        ...
        try {
            // الطلبات المقيدة التي تحتوي على مواد لم تُرجع بالكامل
            $query = Order::where('status', 'confirmed')
                ->whereHas('items', function ($q) {
                    $q->where('quantity', '>', 0);
                });

            // فلتر الصلاحيات للمجهز
            if ($user->isSupplier()) {
                $accessibleWarehouseIds = $user->warehouses->pluck('id')->toArray();
                $query->whereHas('items.product', function ($q) use ($accessibleWarehouseIds) {
                    $q->whereIn('warehouse_id', $accessibleWarehouseIds);
            ...
            // تطبيق الفلاتر (البحث والتاريخ والمندوب/المجهز)
            $this->applyFilters($query, $request);
            ...
            $formattedOrders = $orders->map(function ($order) {
                $item = $this->formatOrderListItem($order);
                // إضافة معلومة إضافية عن المواد المتبقية
                $item['items_with_remaining'] = $order->items->where('quantity', '>', 0)->count();
                return $item;
            });
            ...
            Log::error('MobileAdminOrderController: getPartialReturns failed', ['error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => 'حدث خطأ أثناء جلب الطلبات'], 500);
        }
    }

    /**
     * جلب تفاصيل طلب محدد لغرض الإرجاع الجزئي
     *
     * @param int $id
     * @return \Illuminate\Http\JsonResponse
     */
    public function getPartialReturnOrder($id)
    {
        ...
        try {
            $query = Order::where('id', $id)->where('status', 'confirmed');
            ...
            $order = $query->with(['items.product.primaryImage', 'items.product.warehouse', 'items.size'])->first();

            if (!$order) {
                return response()->json(['success' => false, 'message' => 'الطلب غير موجود أو غير مؤهل للإرجاع'], 404);
            }
            ...
            Log::error('MobileAdminOrderController: getPartialReturnOrder failed', ['error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => 'حدث خطأ أثناء جلب تفاصيل الطلب'], 500);
        }
    }

    /**
     * معالجة الإرجاع الجزئي لطلب
     *
     * @param Request $request
     * @param int $id
     * @return \Illuminate\Http\JsonResponse
    ...
    public function processPartialReturn(Request $request, $id)
    {
        ...
        try {
            $order = Order::where('id', $id)->where('status', 'confirmed')->first();
            ...
            $result = DB::transaction(function () use ($order, $request, $user) {
                $totalReduction = 0;
                $returnNotes = $request->notes ?? 'إرجاع جزئي عبر الموبايل';

                foreach ($request->items as $returnItemData) {
                    $orderItem = OrderItem::where('id', $returnItemData['order_item_id'])
                        ->where('order_id', $order->id)
                        ->firstOrFail();

                    $qtyToReturn = $returnItemData['quantity'];

                    if ($qtyToReturn > $orderItem->quantity) {
                        throw new \Exception("الكمية المراد إرجاعها ($qtyToReturn) أكبر من الكمية المتاحة ({$orderItem->quantity}) للمنتج {$orderItem->product_name}");
                    }

                    // 1. تسجيل حركة الإرجاع
                    ReturnItem::create([
                        'order_id' => $order->id,
                        ...
                        'return_reason' => $returnNotes,
                    ]);

                    // 2. تحديث المخزون
                    if ($orderItem->size) {
                        $orderItem->size->increment('quantity', $qtyToReturn);
                    }
                    ...
                    ProductMovement::record([
                        ...
                        'balance_after' => $orderItem->size ? $orderItem->size->refresh()->quantity : 0,
                        'order_status' => $order->status,
                        'notes' => "إرجاع جزئي من طلب #{$order->order_number}: $returnNotes"
                    ]);

                    // 4. تحديث عنصر الطلب
                    $itemPrice = $orderItem->unit_price;
                    $reduction = $itemPrice * $qtyToReturn;
                    $totalReduction += $reduction;

                    $orderItem->decrement('quantity', $qtyToReturn);
                    $orderItem->decrement('subtotal', $reduction);
                }

                // 5. تحديث إجمالي الطلب
                $order->decrement('total_amount', $totalReduction);

                // 6. إعادة حساب الأرباح
            ...
            return response()->json([
                'success' => true,
                'message' => 'تمت معالجة الإرجاع بنجاح وتحديث المخزون',
            ]);
        } catch (\Exception $e) {
            Log::error('MobileAdminOrderController: processPartialReturn failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            return response()->json(['success' => false, 'message' => 'فشل معالجة الإرجاع: ' . $e->getMessage()], 500);
        }
    }
}
