        ...
        try {
            ...
            if ($httpCode !== 200) {
                ...
                if ($errNum == 21) {
                    ...
                    if ($isMerchantToken) {
                        // Token يبدأ بـ @@ لكن بدون صلاحيات - مشكلة في الحساب نفسه
                        $errorMsg = "❌ الحساب لا يملك صلاحيات API (errNum: 21)\n\n" .
                                   "🔍 التفاصيل:\n" .
                                   "- الحساب المستخدم: {$username}\n" .
                                   "- Token: صحيح (يبدأ بـ @@)\n" .
                                   "- المشكلة: الحساب محدود الصلاحيات\n\n" .
                                   "📝 الحلول المقترحة:\n" .
                                   "1. تأكد أن الحساب هو Merchant Account الرئيسي (وليس Sub-account)\n" .
                                   "2. تواصل مع دعم الواسط لتفعيل صلاحيات API\n" .
                                   "3. تحقق من إعدادات الحساب على موقع الواسط: https://al-waseet.com\n" .
                                   "4. جرب حساب Merchant مختلف إذا كان متوفراً";
                    } else {
                        // Token لا يبدأ بـ @@ - Merchant User Account
                        $errorMsg = "❌ ليس لديك صلاحية للوصول (errNum: 21)\n\n" .
                                   "🔍 السبب: أنت تستخدم Merchant User Account بدلاً من Merchant Account.\n\n" .
                                   "📝 الحل:\n" .
                                   "1. اذهب إلى: /admin/alwaseet/settings\n" .
                                   "2. استخدم بيانات Merchant Account الرئيسي\n" .
                                   "3. اضغط 'إعادة تسجيل الدخول'";
                    }
                }
    ...
    public function getOrdersByIds(array $orderIds, $retry = true)
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                if ($errNum == 21) {
                    $errorMsg = 'ليس لديك صلاحية للوصول. يرجى التحقق من صلاحيات الحساب أو الاتصال بالدعم الفني للواسط.';
                }
    ...
    public function createOrder(array $orderData, $retry = true)
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                // رسالة خطأ محسنة
                if ($errNum == 21) {
                    $errorMsg = 'ليس لديك صلاحية لإنشاء الطلبات. يرجى التحقق من صلاحيات الحساب أو الاتصال بالدعم الفني للواسط.';
                }
